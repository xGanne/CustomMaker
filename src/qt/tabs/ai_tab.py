...
if QT_AVAILABLE:
    class AiTab(QWidget):
        def __init__(self, main_window, parent=None):
            ...
            self.info_label = QLabel("Modo seguro ativo: a IA descreve a edição quando image-edit não está disponível.")
            self.info_label.setWordWrap(True)
            self.prompt_edit = QLineEdit()
            self.prompt_edit.setPlaceholderText("Ex: cabelo loiro, olhos azuis")
            self.apply_button = QPushButton("Aplicar Prompt de Uniforme")
        ...
        def on_apply_click(self):
            ...
            if image is None:
                self.main_window.show_warning("AI", "Carregue uma imagem primeiro.")
                return
            ...
            def on_done(result):
                ...
                if result.kind == "image" and result.image is not None:
                    ...
                    self.main_window.show_info("AI", "Imagem atualizada pela IA.")
                    return
                if result.kind == "text":
                    self.status_label.setText("Concluído em modo seguro.")
                    self.main_window.show_info("Resultado AI", result.text or "A IA não retornou descrição.")
                    return
                self.status_label.setText("Erro")
                self.main_window.show_error("AI", result.error_message or "Erro desconhecido.")

            def on_error(exc):
                logger.exception("Falha no processamento de AI: %s", exc)
                self.apply_button.setEnabled(True)
                self.status_label.setText("Erro")
                self.main_window.show_error("AI", str(exc))

            self.main_window.run_task("qt_ai_apply", "Processando AI", task_fn, on_done=on_done, on_error=on_error)
else:
    class AiTab:
